...
REPO = str(BASE)
DATA = BASE / "data"     # aquí tu controlador guarda los CSV limpios
WORK = BASE / "work"     # parquet intermedios / salida
...
def run_scraping_pipeline(season: str = "2025-2026", ejecutar_r: bool = False) -> None:
    ...
    """
    ctrl.run(season=season, out_dir=str(DATA), ejecutar_r=ejecutar_r)
...
def spark_compute_scores(input_parquet,output_parquet) -> str:
    ...
    calcular(input_parquet=inp, output_parquet=out)
    return out
